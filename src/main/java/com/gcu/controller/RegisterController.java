...
public class RegisterController {
	...
	public String doRegister(@Valid UserModel user, BindingResult bindingResult, Model model) {
		...
		try {
			...
			security.authenticateRegister(user.getCredentials().getUsername(), user.getCredentials().getPassword(),
			...
			System.out.println(String.format(
					"New User created with %s user name, %s password, %s %s name, %s phone number, %s email",
					user.getCredentials().getUsername(), user.getCredentials().getPassword(), user.getFirstname(),
